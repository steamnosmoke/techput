      ...
      <div className="container">
        ...
        <div className="grid grid-cols-3 max-sm:flex max-sm:flex-col gap-7.5 max-sm:gap-4 justify-center">
          ...
          <div className="col-span-3 flex max-sm:flex-col w-full shadow-[0px_0px_8px_2px_rgba(0,0,0,0.1)] rounded-[45px] max-sm:rounded-3xl p-12 max-sm:px-0 max-sm:py-4 items-center justify-between gap-6 max-sm:gap-0">
            ...
            <div className="right w-[50%] max-sm:w-full h-122.5 max-sm:h-auto rounded-[35px] max-sm:rounded-none p-8 max-sm:p-4 pt-15 max-sm:pt-4 bg-orange flex max-sm:flex-col justify-center max-sm:translate-y-5 max-sm:rounded-b-3xl">
              <img src={large} alt="" className="relative w-full max-sm:max-w-125 z- 8" />

              <Link
                to={"/buy-access"}
                className="relative hidden max-sm:block mt-3 mx-5 z-10 w-full"
              >
                <Button
                  text={"Посмотреть курс"}
                  twclass="!w-full !ml-[-20px] !text-orange !text-lg !font-semibold !border-2 !px-6 !py-2 !bg-white"
                />
              </Link>
            </div>
          </div>
